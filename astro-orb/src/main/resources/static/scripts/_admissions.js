...
function postdata() {
  ...
  for (var i = 0; i < criteriaList.length; i++) {
    var jsonObject = {
      id: Number(id),
      criteria: criteriaList[i].getElementsByClassName(
        "newadmissionCriteria"
      )[0].value,
      value:
        criteriaList[i].getElementsByClassName("newcriteriaValue")[0].value,
      operand:
        criteriaList[i].getElementsByClassName("newcriteriaOperand")[0].value,
    };
    resultlist.push(jsonObject);
  }
  for (var i = 0; i < categoryList.length; i++) {
    let salesStart = categoryList[i]
      .getElementsByClassName("salesPeriodDatePicker")[0]
      .value.split(" - ")[0];
    var jsonObject2 = {
      id: Number(id),
      applicationFormAmount:
        categoryList[i].getElementsByClassName("newFormPricetxt")[0].value,
      applicationFormType:
        categoryList[i].getElementsByClassName("newFormTypetxt")[0].value,
      applicationFormQNT:
        categoryList[i].getElementsByClassName("newFormQNTtxt")[0].value,
      paymentMedium: categoryList[i].getElementsByClassName(
        "newFormPaymentOptiontxt"
      )[0].value,
      commencement: convertToISO(
        categoryList[i]
          .getElementsByClassName("salesPeriodDatePicker")[0]
          .value.split(" - ")[0]
      ),
      closure: convertToISO(
        categoryList[i]
          .getElementsByClassName("salesPeriodDatePicker")[0]
          .value.split(" - ")[1]
      ),
      appointmentPerDay:
        categoryList[i].getElementsByClassName("appointmentPerDay")[0].value,
      appointmentCommencement: convertToISO(
        categoryList[i]
          .getElementsByClassName("interviewPeriodDatePicker")[0]
          .value.split(" - ")[0]
      ),
      appointmentClosure: convertToISO(
        categoryList[i]
          .getElementsByClassName("interviewPeriodDatePicker")[0]
          .value.split(" - ")[1]
      ),
    ...
    var bar = new Promise((resolve, reject) => {
      data.applicationCategoryList.forEach((d, index, array) => {
        var details =
          ...
          d.id +
          "> <td>" +
          d.applicationFormType +
          " </td> <td> " +
          d.applicationFormQNT +
          "</td> <td> " +
          d.applicationFormAmount +
          "</td> <td> " +
          d.paymentMedium +
          "</td> <td> " +
          d.commencement.split("T")[0] +
          " \nTo\n " +
          d.closure.split("T")[0] +
          "</td> <td> " +
          d.appointmentCommencement.replace("T", " ") +
          " \nTo\n  " +
          d.appointmentClosure.replace("T", " ") +
          "</td><td>" +
          d.appointmentPerDay +
          "</td> </tr>";
