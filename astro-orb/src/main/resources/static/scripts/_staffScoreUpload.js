...
$('.splash').css({'display': 'block', 'background': '#ffffff3d'}).find('h1, p').remove();
    var jso = {
      ...
      studentClass: document.querySelector(".classSelect").value,
       ...
       return fetchPost("getAssessmentList", jso).then(function (result) {
       if(result.length>0){
...
/*
    async function fetchInstitutionClasses(v){
      ...
      return  fetchPost("getInstitutionClasses",instRequest)
...
async function getStaffSubject(result1) {
  ...
  return fetchPost("getStaffByStaffId", instRequest).then(function (result) {
...
function populateSubjectsOptions(data) {
  ...
  var instRequest = { val: v };
  return fetchPost("getInstitutionSubjects", instRequest).then(function (result) {
  ...
  const filteredResults = result.filter(result => {
    return data.some(item => {
      ...
      const subjectMatch = result.name.toLowerCase() === item.subject.toLowerCase();

      // Match by class group (extract numeric part from subjectClass).match(/\d+/)?.
      const classGroupMatch = splitAtNth(result.classGroupName," ",2)[0].toLowerCase().includes(item.subjectClass.split(" ")[1].toLowerCase());

      return subjectMatch && classGroupMatch;
    });
  });

  var existingSubjects = new Set();

  filteredResults.forEach(function (d) {
    var subjectValue = d.id;

    // Add only if not already in the Set
    if (!existingSubjects.has(subjectValue)) {
      var details = $("<option>").val(subjectValue).text(d.name + " [ " + d.classGroupName + " ] ");
...
function splitAtNth(str, delimiter = " ", n = 1) {
  let count = 0;
  let index = -1;
  let fromIndex = 0;
  ...
  while (count < n) {
    ...
    if (index === -1) return [str]; // Not enough delimiters
    count++;
    fromIndex = index + 1;
  }

  return [
    str.slice(0, index),     // Everything before the nth delimiter
    str.slice(index + 1)     // Everything after
  ];
}