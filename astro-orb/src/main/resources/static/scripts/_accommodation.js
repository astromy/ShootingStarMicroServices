...
type = $('[name="type"]').val();
//fetchLookup();
fetchInstitutionAccommodation(instId.split(",")[0]);

window.copyrights();
$(".saveBlock").click(async function () {
...
async function fetchInstitutionAccommodation(instId) {
    var v = instId.replace(/[\[\]']+/g, "");
    v = v.replace(/\//g, "");
    var instRequest = {val: v};
    return fetchPost("getInstitutionAccommodation", instRequest).then(function (
        result
    ) {
