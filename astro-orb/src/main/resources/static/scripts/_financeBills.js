...
function postdata() {
  ...
  for (var i = 0; i < billsMother.length; i++) {
    ...
    var jsonObject = {
      ...
      bill_Amount:
        billsMother[i].getElementsByClassName("newBillAmount")[0].value,
      creation_Date: "",
      bill_Description:
        billsMother[i].getElementsByClassName("newBillDescription")[0].value,
...
function addEvents() {
  ...
  for (let i = 0; i < billContents.length; i++) {
    billContents[i].addEventListener("click", function (event) {
      ...
      const billName = clickedElement
        .getElementsByTagName("h3")[0]
        .innerHTML.trim();
      const billAmount = clickedElement
        .getElementsByTagName("strong")[0]
        .innerHTML.trim();
      const billDescription = clickedElement
        .getElementsByClassName("billDescription")[0]
        .innerHTML.trim();
      const billCat = clickedElement
      ...
      document.getElementsByClassName("newBillDescription")[0].value =
        billDescription;
      document.getElementsByClassName("newBillDescription")[0].text =
        billDescription;
