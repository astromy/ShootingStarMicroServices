...
async function displayReport(data) {
    try {
        ...
        const fieldMapping = [
            "no", "studentId", "studentName", "studentClass", "subject",
            "classScore", "examsScore", "totalScore", "grade", "position"
        ];

        let tbody = $("#reportTableBody");
        tbody.empty(); // Clear existing rows

        if (!scoreJson.studentReportResponseList || scoreJson.studentReportResponseList.length === 0) {
            tbody.append("<tr><td colspan='10'>No student reports available</td></tr>");
            return;
        }
        ...
        $.each(scoreJson.studentReportResponseList, function (_, student) {
            ...
            if (assessments.length === 0) {
                rows.push(`
                    <tr>
                        ...
                        <td>${student.firstName} ${student.otherName || ""} ${student.lastName}</td>
                        <td></td>
            ...
            assessments.forEach((assessment, index) => {
                rows.push(`
                    <tr>
                        ${index === 0 ? `
                            <td rowspan="${assessments.length}">${studentCounter++}</td>
                            <td rowspan="${assessments.length}">${student.studentId}</td>
                            <td rowspan="${assessments.length}">${student.firstName} ${student.otherName || ""} ${student.lastName}</td>
                            <td rowspan="${assessments.length}">${assessment.studentClass}</td>
                        ` : ""}
...
function generatePDF(assessments) {
    ...
    studentReports.forEach((student) => {
        ...
        terminalReport.text(institution.name.toUpperCase(), offset + logoWidth + 22.5, offset + 20);
        terminalReport.setFontSize(9);
        terminalReport.setFont("helvetica", "normal");
        terminalReport.setTextColor("#058709");
        terminalReport.text("..." + institution.slogan, offset + logoWidth + 22.5, offset + 24);

        // ** Institution Contact Details **
        terminalReport.setFontSize(9);
        terminalReport.text(institution.postalAddress , offset +  xcord+(65*2), offset + 32);
        terminalReport.setFontSize(9);
        terminalReport.text("Email: " + institution.email, offset +  xcord+(65*2), offset + 36);
        terminalReport.setFontSize(9);
        terminalReport.text("Website: " + institution.website, offset +  xcord+(65*2), offset + 40);
        terminalReport.setFontSize(9);
        terminalReport.text(institution.city +" "+ institution.region +" " + institution.country, offset + xcord+(65*2), offset + 44);

        // ** Add Student Details **
        terminalReport.setFontSize(9);
        terminalReport.setTextColor("#000");
        terminalReport.text(`Student ID: ${student.studentId}`, xcord, ycord+4);
        terminalReport.text(`Student Name: ${student.firstName} ${student.otherName || ""} ${student.lastName}`, xcord, ycord+(4*2));
        terminalReport.text(`Gender: ${student.gender}`, xcord, ycord+(4*3));
        terminalReport.text(`Class: ${student.studentAssessment[0]?.studentClass || "N/A"}`, xcord, ycord+(4*4));
        terminalReport.text(`Term: ${student.studentAssessment[0]?.term || "N/A"}`, xcord, ycord+(4*5));
        terminalReport.text(`Academic Year: ${student.studentAssessment[0]?.academicYear || "N/A"}`, xcord, ycord+(4*6));

        // ** Add Class Details **
        terminalReport.setFontSize(9);
        terminalReport.setTextColor("#000");
        terminalReport.text("Program of Study:" + institution.classGroup, xcord+(65), ycord+(4*1));
        terminalReport.text(`Student Class: ${student.studentAssessment[0]?.studentClass || "N/A"}`, xcord+(65), ycord+(4*2));
        terminalReport.text(`Class Size: ${studentReports.length}`, xcord+(65), ycord+(4*3));
        terminalReport.text("Class Average Mark:" + institution.classAverage, xcord+(65), ycord+(4*4));
        ...
        terminalReport.setFontSize(9);
        terminalReport.setTextColor("#000");
        terminalReport.text(`Student Average Grade: ${student.averageGrade || "N/A"}`, xcord+(65*2), ycord+(4*1));
        terminalReport.text(`Student General Remarks: ${student.averageRemark || "N/A"}`, xcord+(65*2), ycord+(4*2));
        terminalReport.text(`Student Average Mark: ${student.averageScore || "N/A"}`, xcord+(65*2), ycord+(4*3));
        terminalReport.text(`Student Average Position: ${student.averageScore || "N/A"}`, xcord+(65*2), ycord+(4*4));
        ...
        // ** Table Headers **
        const headers = ["Subject", "Class Score", "Exams Score", "Total Score", "Grade", "Position", "Remarks"];
        ...
        // Add footer
        terminalReport.setFontSize(10);
        terminalReport.text(footerText, 10, terminalReport.internal.pageSize.height - 10);
        });

    // ** Save or Download PDF **
    terminalReport.save(studentReports[0].studentAssessment[0]?.studentClass + " Terminal Report.pdf");
}