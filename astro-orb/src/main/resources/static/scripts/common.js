...
depGroups.forEach((anchor) => {
    ...
    if (
        [...permissionGroups].some((permission) => plainText.includes(permission))
    ) {
        anchor.style.removeProperty("display"); // Remove display style on match
    } else {
        anchor.style.display = "none"; // Explicitly hide unmatched anchors
...
function removeUnwantedResources(tagName) {
    ...
    var lst = [];
    lst = Array.from(mother.getElementsByTagName(tagName)).filter(
        (el) => el.getAttribute("data-dynamic") === "true"
    );
    ...
    if (mother.getElementsByClassName("sweet-overlay").length > 1) {
        mother.removeChild(
            mother.getElementsByClassName("sweet-overlay")[1].parentElement
        );
    }
}
...
function broadSheetBuild() {
    ...
    const newScripts = [
        "vendor/sweetalert/lib/sweet-alert.min.js",
        "scripts/subscripts/broadSheet.js",
    ];
    const newLinks = [
        "vendor/sweetalert/lib/sweet-alert.css",
        "vendor/metisMenu/dist/metisMenu.css",
...
function pendingApplicatsBuild() {

    // Define new resources specific to this view
    const newScripts = [
        "vendor/sweetalert/lib/sweet-alert.min.js",
        "scripts/subscripts/pendingApplicants.js",
    ];
    const newLinks = [
        "vendor/sweetalert/lib/sweet-alert.css",
        "vendor/metisMenu/dist/metisMenu.css",
...
async function setFileInputFromByteArray(
    base64String,
    el,
    extension,
    fileName
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            for (let i = 0; i < totalChunks; i++) {
                ...
                const byteNumbers = Array.from(atob(chunk), (char) =>
                    char.charCodeAt(0)
                );
...
async function setAllFileTypeInputFromByteArray(base64String, el, fileName) {
    ...
    try {
        ...
        const mimeType = mimeTypeMatch
            ? mimeTypeMatch[1]
            : "application/octet-stream"; // Default to octet-stream if not found
